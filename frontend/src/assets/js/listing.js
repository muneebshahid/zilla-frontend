(function($) {
    ...
    $.extend($.apusThemeCore, {
        ...
        listing_init: function() {
            var self = this;
            this.findgo_listing_opts = {"ajaxurl":"","login_url":"","strings":{"wp-job-manager-file-upload":"Add Photo","no_job_listings_found":"No results","results-no":"Results Found"},"mapbox_token":"","mapbox_style":"","custom_style":"","default_icon":"\/\/\/home\/muazzam\/mywork\/findgo\/wp-content\/themes\/findgo\/images\/default-icon.png","timeFormat":"g:i a","none_option_label":"Closed","none_option_value":"Closed"};
        ...
        bookmarkInit: function() {
            ...
            $( "body" ).on( "click", ".apus-bookmark-add", function( e ) {
                ...
                var url = this.findgo_listing_opts.ajaxurl + '?action=findgo_add_bookmark&post_id=' + post_id;
                var self = $(this);
                self.addClass('loading');
                $.ajax({
                    url: url,
            ...
            $( "body" ).on( "click", ".apus-bookmark-added", function( e ) {
                ...
                var post_id = $(this).data('id');
                var url = this.findgo_listing_opts.ajaxurl + '?action=findgo_remove_bookmark&post_id=' + post_id;
                var self = $(this);
                self.addClass('loading');
                $.ajax({
                    url: url,
        ...
        mapInit: function() {
            ...
            if ($('.no_job_listings_found').length) {
                $('<div class="results">' + this.findgo_listing_opts.strings['no_job_listings_found'] + '</div>').prependTo('.showing_jobs, .listing-search-result');
            }
            ...
            if ( this.findgo_listing_opts.mapbox_token != '' ) {
                var tileLayer = L.tileLayer('https://api.tiles.mapbox.com/v4/' + this.findgo_listing_opts.mapbox_style + '/{z}/{x}/{y}.png?access_token=' + this.findgo_listing_opts.mapbox_token, {
                    maxZoom: 18,
        ...
        previewInit: function() {
            ...
            $('a.listing-preview').on('click', function (e) {
                ...
                var url = this.findgo_listing_opts.ajaxurl + '?action=findgo_preview_listing&listing_id=' + listing_id;
